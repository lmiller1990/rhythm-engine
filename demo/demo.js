...
function visualize(chart) {
    ...
    for (var _i = 0, _a = chart.notes; _i < _a.length; _i++) {
        var note = _a[_i];
...
fromEvent(window, 'keydown')
    ...
    var timing = judge(input, note);
    var $note = document.querySelector("[data-noteid=\"" + note.id + "\"]");
    // early
    if (timing > 0) {
...
function gameLoop(ms) {
    ...
    var nextMs = performance.now() - initOffset;
    requestAnimationFrame(function () { return gameLoop(nextMs); });
}
visualize(chart);
setTimeout(function () {
    requestAnimationFrame(function () { return gameLoop(0); });
}, initOffset);
